...
export default function StudentQueue({
  ...
  // Surface the student's queue position in the browser tab title so they can
  // watch their spot from another tab.
  useEffect(() => {
    if (
      studentQuestion?.status === OpenQuestionStatus.Queued &&
      studentQuestionIndex >= 0
    ) {
      document.title = `(${studentQuestionIndex + 1}) ${
        queue?.room ?? ""
      } Queue | Khoury Office Hours`;
    }
  }, [studentQuestion, studentQuestionIndex, queue]);
