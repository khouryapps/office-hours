...
export class ProfileService {
  ...
  public async hasUnregisteredCourses(userId: number): Promise<boolean> {
    const profCourses = await ProfSectionGroupsModel.findOne({
      where: { profId: userId },
    });
    if (!profCourses) return false;
    ...
    for (const c of profCourses.sectionGroups) {
      if (c.crns.length !== 0) {
        ...
        if (!profCourse) return true;
      }
    }
    return false;
  }

  public async resetRegistration(userId: number): Promise<void> {
    const lastRegistered = await LastRegistrationModel.findOne({
      where: { profId: userId },
    });
    if (lastRegistered) {
      await lastRegistered.remove();
    }
  }
